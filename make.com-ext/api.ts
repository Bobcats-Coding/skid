...
export const createListScenarioRequest = ({
    ...
    organizationId,
    teamId,
  }: { accessToken: string, organizationId?: string, teamId?: string }): ScenarioApiListRequest => {
    const search: AuthSearchParams = organizationId !== undefined
      ? { organizationId }
      : teamId !== undefined
        ? { teamId }
        : {};
...
export const createGetScenarioRequest = ({
    scenarioId,
    accessToken,
}: { scenarioId: number, accessToken: string }): ScenarioApiSingleRequest => ({
    method: 'GET',
...
export const createScenarioActionRequest = ({
    ...
    scenarioId,
    accessToken,
}: { action: SCENARIO_ACTIONS, scenarioId: number, accessToken: string }): ScenarioApiActionRequest => ({
    method: 'POST',
